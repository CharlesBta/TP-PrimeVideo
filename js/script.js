...
const buttons = {
    dailyTrending: document.querySelector('#td'),
    weeklyTrending: document.querySelector('#week'),
    topRatedTVShows: document.querySelector('#top'),
    popularTVShows: document.querySelector('#pop')
};

buttons.dailyTrending.addEventListener('click', () => {
    fetchData('https://api.themoviedb.org/3/trending/all/day?language=fr-FR', 'tendance', 'movie');
    buttons.dailyTrending.classList.add('active');
    buttons.weeklyTrending.classList.remove('active');
});

buttons.weeklyTrending.addEventListener('click', () => {
    fetchData('https://api.themoviedb.org/3/trending/all/week?language=fr-FR', 'tendance', 'movie');
    buttons.weeklyTrending.classList.add('active');
    buttons.dailyTrending.classList.remove('active');
});

buttons.topRatedTVShows.addEventListener('click', () => {
    fetchData('https://api.themoviedb.org/3/tv/top_rated?language=fr-FR', 'Series', 'tv');
    buttons.topRatedTVShows.classList.add('active');
    buttons.popularTVShows.classList.remove('active');
});

buttons.popularTVShows.addEventListener('click', () => {
    fetchData('https://api.themoviedb.org/3/tv/popular?language=fr-FR', 'Series', 'tv');
    buttons.popularTVShows.classList.add('active');
    buttons.topRatedTVShows.classList.remove('active');
});
